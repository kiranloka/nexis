...
use crate::{state::*,errors::PredictionMarketError};
...
pub fn create_markte(
    ...
    require!(options.len()>=2 && options.len()<=10,PredictionMarketError);
    require!(deadline>Clock::get()?.unix_timestamp,PredictionMarketError);
